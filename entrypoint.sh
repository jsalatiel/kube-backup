...
for namespace in $NAMESPACES; do
    ...
    for type in $RESOURCETYPES; do
        ...
        'del(
            ...
            .spec.clusterIP,
            .status
        )' | python -c 'import sys, yaml, json, ruamel.yaml ; yaml = ruamel.yaml.YAML() ; yaml.explicit_start = True ; data = json.load(sys.stdin,object_pairs_hook=ruamel.yaml.comments.CommentedMap) ; ruamel.yaml.scalarstring.walk_tree(data); yaml.dump(data, sys.stdout)' >"$GIT_REPO_PATH/$GIT_PREFIX_PATH/${namespace}/${name}.${type}.yaml"
        done
    done
done
